...
import { askQuestion as askQuestionAPI, askMultiFiles, fetchPreviewData } from '@/lib/api'
...
export default function ChatPage() {
  ...
  const isMulti = pathname.includes('/chat/multi')
  ...
  useEffect(() => {
    if (isMulti) {
      ...
      const files = filesParam.split(',').map(decodeURIComponent)
      setFileIds(files)

      // 🔁 For demo: just fetch preview of first file
      fetchPreviewData(files[0]).then((res) => {
  ...
  }, [pathname, searchParams, params, isMulti])
    ...
    <main className="relative min-h-screen bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] text-white px-6 pt-28 pb-20 overflow-x-hidden">
      ...
      <div className="relative z-10 mx-auto px-4 space-y-10 max-w-[1400px]">
            ...
            <div className="w-full overflow-x-auto">
              <table className="table-auto min-w-[800px] text-sm text-left border-collapse">
                <thead>
                  <tr>
                    {columns.map((col) => (
                      <th key={col} className="px-4 py-2 bg-white/10 text-gray-300 font-medium border-b border-white/10 text-sm max-w-[240px] truncate">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {preview.map((row, i) => (
                    <tr key={i} className="border-b border-white/10 hover:bg-white/10 transition">
                      {columns.map((col) => (
                        <td key={col} className="px-4 py-2 text-gray-100 align-top max-w-[240px] truncate" title={String(row[col])}>
                          {String(row[col])}
                        </td>
                      ))}
                    </tr>
        ...
        <div className="w-full max-w-[1000px] mx-auto rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 shadow-lg space-y-4">
          <ChatBox
            fileId={isMulti ? fileIds : fileIds[0]} 
            onAsk={isMulti ? askMultiFiles : askQuestionAPI}
          />
        </div>
      </div>
    </main>
